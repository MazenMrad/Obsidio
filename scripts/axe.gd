...
@export var min_trail_width: float = 0.7
@export var max_trail_width: float = 3.5
@export var speed_for_max_width: float = 900.0
@export var trail_tail_offset: float = 10.0
@export var stick_lifetime: float = 2.2
@export var ground_embed_distance: float = 3.0
@export var ground_stick_time: float = 0.07
...
func _start_fade_out() -> void:
	...
	tw.tween_property(trail, "modulate", Color(trail.modulate.r, trail.modulate.g, trail.modulate.b, 0.0), 0.32).set_trans(Tween.TRANS_LINEAR)
	tw.parallel().tween_property(trail, "width", 0.0, 0.32)
